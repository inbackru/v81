...
class Referral(db.Model):
    ...
    bonus_amount = db.Column(db.Numeric(15, 2), default=Decimal('20000.00'))
    status = db.Column(db.String(20), default='completed')
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    bonus_credited_at = db.Column(db.DateTime, nullable=True)
...
class Partner(UserMixin, db.Model):
    ...
    partner_id = db.Column(db.String(20), unique=True, nullable=False)
    referral_code = db.Column(db.String(8), unique=True, nullable=False, index=True)

    referred_by_id = db.Column(db.Integer, db.ForeignKey('partners.id'), nullable=True)
    ...
    total_earned = db.Column(db.Numeric(15, 2), default=Decimal('0.00'), nullable=False)
    total_withdrawn = db.Column(db.Numeric(15, 2), default=Decimal('0.00'), nullable=False)
    ...
    referred_by = db.relationship('Partner', remote_side=[id], backref='referrals')

    def __init__(self, **kwargs):
        super(Partner, self).__init__(**kwargs)
        if not self.partner_id:
            self.partner_id = self.generate_partner_id()
        if not self.referral_code:
            self.referral_code = self.generate_referral_code()

    def generate_partner_id(self):
        import random
        while True:
            pid = f"PRT{random.randint(10000000, 99999999)}"
            if not Partner.query.filter_by(partner_id=pid).first():
                return pid
    ...
    def generate_referral_code(self):
        ...
        while True:
            code = ''.join(secrets.choice(string.ascii_uppercase + string.digits) for _ in range(8))
            if not Partner.query.filter_by(referral_code=code).first():
                return code
    ...
    def full_name(self):
        ...
        return ' '.join(parts) if parts else 'Партнёр'

    def __repr__(self):
        return f'<Partner {self.phone}>'
...
class PartnerReferral(db.Model):
    ...
    referred_partner_id = db.Column(db.Integer, db.ForeignKey('partners.id'), nullable=True)
    referred_user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)

    bonus_amount = db.Column(db.Numeric(15, 2), default=Decimal('0.00'), nullable=False)
    ...
    partner = db.relationship('Partner', foreign_keys=[partner_id], backref='partner_referral_records')
    referred_partner = db.relationship('Partner', foreign_keys=[referred_partner_id])
    referred_user = db.relationship('User', backref='partner_referral_records')

    def __repr__(self):
        return f'<PartnerReferral partner={self.partner_id} level={self.level}>'
...
class PartnerWithdrawal(db.Model):
    ...
    def __repr__(self):
        return f'<PartnerWithdrawal {self.amount} status={self.status}>'
